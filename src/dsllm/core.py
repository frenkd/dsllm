...
from .exceptions import GenerationError, RetryExhaustedError, ValidationError
from .types import DSLGenerator, GenerationRequest, GenerationResult, LLMProvider
        ...
        if self.enable_validation:
            ...
            if not pre_flight.is_valid:
                raise ValidationError(
                    f"Pre-flight validation failed: {pre_flight.errors}"
                )

        # Generate with retries
        try:
            result: GenerationResult = await self._generate_with_retries(request)
            return result
        except Exception as e:
            logger.error(f"Generation failed after retries: {e}")
            raise RetryExhaustedError(
                f"Failed to generate valid DSL after {max_retries} attempts"
            ) from e
