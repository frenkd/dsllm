...
class SQLPromptGenerator:
    ...
    def get_system_prompt(self, enforce_read_only: bool = False) -> str:
        ...
        if enforce_read_only:
            read_only_info = (
                " ONLY generate SELECT statements. Do not generate any data "
                "modification statements."
            )

        return (
            f"You are an expert SQL developer. Generate syntactically correct "
            f"SQL statements based on natural language descriptions."
            f"{read_only_info}\n\n"
            "Rules:\n"
            ...
            "If the request is ambiguous, make reasonable assumptions based on "
            "common SQL patterns."
        )
    ...
    def format_user_prompt(self, request: GenerationRequest) -> str:
        ...
        """
        prompt_parts = [f"Generate a SQL statement for: {request.natural_language}"]
        ...
        if request.context:
            ...
            if "tables" in request.context:
                tables_info = "\n".join(
                    [f"- {table}" for table in request.context["tables"]]
                )
                prompt_parts.append(f"\nAvailable tables:\n{tables_info}")
        ...
        if request.constraints:
            ...
            if "max_rows" in request.constraints:
                constraints_text.append(
                    f"Limit results to {request.constraints['max_rows']} rows"
                )
